...
const SupportPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 sm:py-12 px-4">
      ...
      >
        <HelpCircle className="h-14 w-14 sm:h-20 sm:w-20 text-primary mx-auto mb-4 sm:mb-6" />
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-3 sm:mb-4 gradient-text">Centro de Soporte</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
        {contactOptions.map((option, index) => (
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="p-5 sm:p-8 rounded-xl glassmorphism shadow-xl hover:shadow-2xl transition-shadow duration-300 flex flex-col"
          >
            <div className="flex items-center mb-4 sm:mb-6">
              <div className="p-2 sm:p-3 bg-primary/20 rounded-full mr-3 sm:mr-4">
                {option.icon}
              </div>
              <h2 className="text-xl sm:text-2xl font-semibold text-foreground">{option.title}</h2>
            </div>
            <p className="text-sm sm:text-base text-muted-foreground leading-relaxed mb-4 sm:mb-6 flex-grow">{option.description}</p>
            <Button
              onClick={() => window.open(option.actionLink, option.type === "discord" ? '_blank' : '_self')}
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground text-sm sm:text-md py-2 sm:py-3 mt-auto"
            >
              {option.actionText}
      ...
      <motion.div
        ...
        className="mt-8 sm:mt-16 p-5 sm:p-8 rounded-xl glassmorphism shadow-lg text-center"
      >
        <h3 className="text-xl sm:text-2xl font-semibold mb-2 sm:mb-3 text-foreground">Horario de Soporte</h3>
